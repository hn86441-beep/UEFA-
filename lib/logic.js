...
// لاعبي كل فريق المسجّلة مسبقًا. يدعم أيضًا الصيغة القديمة (اسم حر بلا تسجيل مسبق)
// حتى لا تضيع أي أهداف أُدخلت قبل إضافة ميزة قوائم اللاعبين.
export function computeTopScorers(teams, matches) {
  const teamById = Object.fromEntries(teams.map((t) => [t.id, t]));
  const tally = {}; // key: playerKey|teamId -> { name, teamId, teamName, goals }

  function playerName(teamId, s) {
    if (s.playerId) {
      const player = teamById[teamId]?.players?.find((p) => p.id === s.playerId);
      return player?.name?.trim() || null;
    }
    return s.name?.trim() || null; // توافق مع الصيغة القديمة
  }

  function addGoal(teamId, s) {
    const name = playerName(teamId, s);
    if (!name) return;
    const key = `${s.playerId || name}|${teamId}`;
    if (!tally[key]) {
      tally[key] = { name, teamId, teamName: teamById[teamId]?.name || "—", goals: 0, playerId: s.playerId || null };
    }

...
export async function getData() {
  ...
  if (isVercel) {
    throw new Error(
      "لم يتم ربط قاعدة بيانات دائمة بالموقع بعد. من مشروعك في Vercel اذهب إلى Storage → أضف قاعدة بيانات Redis/Upstash → اربطها بالمشروع → أعد النشر."
    );
  }
  return readLocal();
}
...
export async function getSoundClips() {
  if (hasKV) {
    const kv = await getKV();
    const clips = await kv.get(SOUNDS_KEY);
    return clips || {};
  }
